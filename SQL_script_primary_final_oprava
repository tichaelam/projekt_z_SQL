...
-- 7 KROK
price_yearly_by_cat_lag AS (
    SELECT
        year,
        category_code,
        avg_price_yearly_raw,
        LAG(avg_price_yearly_raw) OVER (
            PARTITION BY category_code
            ORDER BY year
        ) AS prev_price_yearly_raw
...
price_yearly_by_cat_yoy_raw AS (
    SELECT
        ...
        category_code,
        avg_price_yearly_raw,
        prev_price_yearly_raw,
        CASE
            WHEN prev_price_yearly_raw = 0 OR prev_price_yearly_raw IS NULL THEN NULL
            ELSE ((avg_price_yearly_raw - prev_price_yearly_raw) / prev_price_yearly_raw * 100)
        END AS price_yoy_raw
    FROM price_yearly_by_cat_lag
),
-- 8 KROK
food_year AS (
    SELECT
...
food_and_wage_lag AS (
    ...
    FROM food_year fy
    JOIN payroll_yearly_avg_overall_raw w
        ON fy.year = w.year
),
food_and_wage_yearly_raw AS (
    SELECT
        year,
        avg_food_price_yearly_raw,
        avg_wage_yearly_overall_raw,
        prev_food,
        prev_wage,
        CASE
            WHEN prev_food IS NULL THEN NULL
            ELSE ((avg_food_price_yearly_raw - prev_food) / prev_food * 100)
        END AS food_yoy_raw,
        CASE
            WHEN prev_wage IS NULL THEN NULL
            ELSE ((avg_wage_yearly_overall_raw - prev_wage) / prev_wage * 100)
        END AS wage_yoy_raw
    FROM food_and_wage_lag
)
-- 9 KROK
SELECT
    p.year,
    ...
    (p.category_code = 111301) AS is_bread,
    (p.category_code = 114201) AS is_milk
FROM price_quarterly_raw p
JOIN payroll_yearly_avg_branch_raw b
    ON p.year = b.year
LEFT JOIN czechia_price_category cpc
    ON p.category_code::text = cpc.code::text
LEFT JOIN czechia_payroll_industry_branch ib
    ON b.industry_branch_code = ib.code
LEFT JOIN price_yearly_by_cat_raw pyc
    ON p.year = pyc.year
   AND p.category_code = pyc.category_code
LEFT JOIN price_yearly_by_cat_yoy_raw pyoy
    ON p.year = pyoy.year
   AND p.category_code = pyoy.category_code
LEFT JOIN food_and_wage_yearly_raw fwy
    ON p.year = fwy.year
